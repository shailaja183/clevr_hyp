...
def object_handler(scene_struct, inputs, side_inputs):
  return [0]
...
def add_abs_handler(scene_struct, inputs, side_inputs):
  assert len(inputs) == 2
  assert len(side_inputs) == 0
  return sorted(list(set(inputs[0]) | set(inputs[1])))

def remove_abs_handler(scene_struct, inputs, side_inputs):
  assert len(inputs) == 2
  assert len(side_inputs) == 0
  return sorted(list(set(inputs[0]) - set(inputs[1])))
...
def make_change_handler(attribute):
  def change_handler(scene_struct, inputs, side_inputs):
    assert len(inputs) == 1
    assert len(side_inputs) == 1
    value = side_inputs[0]
    output = []
    for idx in inputs[0]:
      scene_struct['objects'][idx][attribute] = value
      assert scene_struct['objects'][idx][attribute] == value
      output.append(idx)
    return output
...
execute_handlers = {
  ...
  'change_color': make_change_handler('color'),
  'change_shape': make_change_handler('shape'),
  'change_material': make_change_handler('material'),
  'change_size': make_change_handler('size'),
  'add_abs': add_abs_handler, 
  'remove_abs': remove_abs_handler
}
